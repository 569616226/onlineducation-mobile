<template>
	<div v-title data-title="收藏的课程">
		<yd-layout>
			<yd-infinitescroll :callback="loadList" ref="infinitescrollDemo" v-if="data.length > 0">
				<div slot="list">
					<div class="collect">
						<div class="buyclass" v-for="item,key in data" :key="key">
							<div class="buy_content" @click="ordergoclass('TrainDetail',item.id)">
								<div class="buy_img">
									<img :src="item.pic" />
								</div>
								<div class="buy_details">
									...
									<div class="collect_delete">
										<p class="buy_details_p3" v-if="isNaN(item.price)">{{item.price}}</p>
										<p class="buy_details_p3" v-else>￥{{item.price}}</p>
									</div>
								</div>
								<div class="clear"></div>
							</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getData() {
				this.loading_v = true
				axios.get('/item/guest/my_collect_train')
					.then((res) => {
			...
			delete_collect(id, key) {
				this.$dialog.confirm({
					mes: '真的要取消收藏吗?',
					opts: () => {
						axios.get('/item/train/' + id + '/uncollect')
							.then((res) => {
								if(res.data.status) {
									this.$dialog.toast({
