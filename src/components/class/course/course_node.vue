<template>
	<div>
		<div class="class_top_class">
			<p class="class_down" v-if="data_class.status == '已下架'">由于不可抗拒的原因，本课程已下架！</p>
			<div v-if="data_class.type == '免费课程' || data_class.is_show">
				<div v-if="data_class.sections != null">
			...
			<div v-else>
				<div v-if="data_class.sections[0].is_free == 1">
					<showclass ref="showclass"></showclass>
				</div>
				<div v-else>
					<img :src="data_class.pictrue" style="width: 100%;" />
		...
		<div class="class_teacher">
			<img :src="data_class.teacher.avatar" class="teacher_avatar" />
		...
		<div class="class_content" v-if="data_class.type == '免费课程'">
			...
			<div v-if="data_class.sections != null">
				<div class="class_content_top_gl" v-for="item,key in data_class.sections" v-if="key < datalength" :key="key" @click="show_class(item,key)">
					<div class="class_content_left class_col_span left">
						<span>{{key + 1}}</span>
					</div>
					<div class="class_content_right right">
						<div class="left width80">
							<p ref="showing" class="class_content_p" v-if="item.is_learned == 0">{{item.name}}</p>
							<p ref="showing" class="class_content_p gray" v-if="item.is_learned == 1">{{item.name}}</p>
							<span class="class_content_time">{{item.duration}}</span>
						</div>
					</div>
					<div class="clear"></div>
				</div>
		...
		<!--
        	...
        	描述：vip提示
        -->
		<yd-popup v-model="show_vip" position="center" width="80%" class="show_vip_popup">
		...
		<!--
        	...
        	描述：收费提示
        -->
		<yd-popup v-model="show_course" position="center" width="80%" class="show_vip_popup">
		...
		methods: {
			...
			show_class(item, key) {
				...
				if(this.data_class.type == '免费课程') {
					this.$refs.showing.forEach((item, key) => {
						item.style.color = ""
					})
					this.$refs.showing[key].style.color = "rgb(255, 112, 67)"
					this.$refs.showclass.getData(id)
				}
			...
			showplaying() {
				...
				this.$refs.showclass.play_fixed()
			}
		},
		computed: {
