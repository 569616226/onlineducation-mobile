<template>
	<div>
		<div id="id_test_video" style="width:100%; height:220px;position: relative; z-index: 9999;" ref="player"></div>
...
<script>
	...
	export default {
		...
		methods: {
						...
						if(msg.type == 'load'){
							that.$localStorage.get('currentTime').forEach((item) => {
								if(item.id == that.data.id) {
									if(item.time > 0) {
										console.log(1)
										that.time_str = that.formatSeconds(item.time)
										that.show_div = true
									}
								}
							})
			...
			getData(id) {
				axios.get('/item/lesson/section/' + id)
					.then((res) => {
						if(res.data.status == false) {
							...
							if(this.play_video.length != 0){
								this.play_video.destroy()
